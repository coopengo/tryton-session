...
function rpc (session, method, params, context, options) {
  ...
  if (!options.anonymous) {
    req.set('Authorization', session.authorization())
  }
  if (context) {
    params = _.slice(params)
    params.push({...context, auto_accept_warnings:true})
  }
      ...
      if (err) {
        ...
        if (!body) {
          ...
          if (body.error[0].match(/\d\d\d:/)) {
            ...
            status = 0
            if (_.includes(['UserError', 'UserWarning'], body.error[0])) {
              message = body.error[1].join(' - ')
            } else {
